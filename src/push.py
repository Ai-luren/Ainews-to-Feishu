...
def main() -> int:
    ...
    if not failed:
        record_last_run(STATE_PATH, status="ok")
        _log("[meta] all completed")
        return 0
    if any(ok for _, ok in attempted):
        record_last_run(STATE_PATH, status="partial", error=",".join(failed))
        _log(f"[meta] partial success, failed: {','.join(failed)}", err=True)
        return 0
    record_last_run(STATE_PATH, status="failed", error=",".join(failed))
