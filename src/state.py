...
def load_state(path: Path) -> Dict[str, Any]:
    ...
    data.setdefault("last_run_status", None)
    data.setdefault("last_run_error", None)
    return data
...
def record_last_run(path: Path, status: str, error: Optional[str] = None) -> None:
    """记录最后一次运行状态。
...
def get_last_run(path: Path) -> Optional[Dict[str, Any]]:
    """获取最后一次运行状态。"""
    data = load_state(path)
