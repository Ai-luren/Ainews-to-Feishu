...
def render_card(daily: dict) -> dict:
    ...
    # 构建内容 markdown
    md_lines = [f"**{len(tweets)} 条热门推文 · 来自 {total_builders} 位 AI builder**\n"]
    ...
    for tweet in tweets:
        ...
        md_lines.append(f"**{name}** @{handle}")
        if text_zh and text_zh != tweet.get("text", ""):
            md_lines.append(f"  {text_zh}")
        md_lines.append(f"  {text_en}")
        md_lines.append(f"  ❤️ {likes}  🔁 {retweets}  [↗]({url})\n")

    md_content = "\n".join(md_lines)

    # 底部来源
    footer = "内容来自 [follow-builders](https://github.com/zarazhangrui/follow-builders)，中文由 Google Translate 翻译"
